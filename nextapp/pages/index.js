...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const periodicCalls = setInterval(async () => {
      ...
      if (_isPresaleStarted) {
        ...
        if (_isPresaleEnded) clearInterval(periodicCalls);
      }
    }, 5000);
    setInterval(async () => {
      getMintedTokensIDs();
    ...
    if (isContractOwner()) renderWithdrawButton();
  ...
  const withdrawAmount = async () => {
    try {
      ...
      if (hasAmount) {
        ...
        window.alert(
          "Contract amount withdrawn successfully and moved to the owner's wallet"
        );
      } else {
        window.alert(
          "Contract's balance is empty. There is no amount to be withdrawn"
        );
      }
    } catch (error) {
      console.error(error);
  ...
  const renderWithdrawButton = () => {
    if (isOwner)
      return (
        <button className={styles.button} onClick={withdrawAmount}>
          Withdraw Contract Amount 💰{" "}
        </button>
      );
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.main}>
        <div>
          ...
          <div className={styles.description}>{renderNFTButton()}</div>
          <div className={styles.description}>{renderWithdrawButton()}</div>
        </div>
